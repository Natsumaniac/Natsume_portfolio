...
- **Technologies**: Stored as a Textarea (one item per line), parsed via `natsume_portfolio_parse_lines()` helper
- **Gallery Images**: Individual image fields (`work_gallery_1`, `work_gallery_2`, `work_gallery_3`) collected via `natsume_portfolio_get_work_gallery()`
